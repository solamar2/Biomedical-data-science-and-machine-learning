function [theta,J,theta_mat] = LinearRegRidge(X,y,p,alpha,lambda)
...
for j=1:l %number of features + 1 
    for i = 1:m
        if j==1
            ...
            theta_temp(j) = theta_temp(j)-alpha*(1/m)*(h_x(i)-y(i))*X(i,j)-alpha*lambda*2*theta_temp(j); %update function
        end
    end
end
   ...
   J(k) = 1/(2*m)*sum((h_x-y).^2)+lambda*sum(theta(2:end).^2); %cost function
end
end
