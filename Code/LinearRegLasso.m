function [theta,J,theta_mat] = LinearRegLasso(X,y,p,alpha,lambda)
...
for j=1:l %number of features + 1 
    for i = 1:m
         if j==1
            theta_temp(j) = theta_temp(j)-alpha*(1/m)*(h_x(i)-y(i))*X(i,j); %update function 
             ...
             theta_temp(j) = theta_temp(j)-alpha*(1/m)*(h_x(i)-y(i))*X(i,j)-alpha*lambda*sign(theta_temp(j)); %update function
        end
    end
end
   ...
   J(k) = 1/(2*m)*sum((h_x-y).^2)+lambda*sum(abs(theta(2:end))); %cost function
end
end
